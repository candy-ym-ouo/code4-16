...
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
- 按库位冻结的盘点任务、逐批实盘录入、提交时重算差异并一次性核销。
- 颜色变化时间线和批次当前颜色。
...
- 盘点创建即冻结库位：冻结期间该库位批次的调整、消耗、撤销、移动、归档一律拒绝（业务校验 + 数据库触发器双重保护），其它库位不受影响。
- 盘点差异在提交时以冻结账面快照重算：盘盈写 `STOCKTAKE_IN`、盘亏写 `STOCKTAKE_OUT`，提交只成功一次。
- 盘点提交必须携带幂等键，重复提交只回放首次结果，不二次调账、不覆盖审计。
