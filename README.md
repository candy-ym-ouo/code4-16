...
验收脚本会真实创建来源、材料、批次、项目、需求、消耗和颜色变化，验证余额从 1000g 降到 500g、撤销后恢复 1000g、最新流水为 `REVERSAL`、颜色和搜索正确；并创建盘点任务验证库位冻结、实盘录入、差异核销（200g 核销为 180g）和重复提交不产生二次调整。
...
- 盘点任务创建即冻结所选库位，冻结期间禁止消耗、调整、移库、入库和归档。
- 盘点提交时按批次当前余额重算差异并核销，重复提交不会二次调整或覆盖审计。
