...
盘点以**库位**为冻结单位：创建盘点单即冻结该库位，逐批录入实盘数，提交时在同一事务内按冻结账面快照重算差异并核销。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET/POST | `/stocktakes` | 查询盘点单，或创建并冻结库位 |
| GET | `/stocktakes/:id` | 盘点单详情（含逐批行项目） |
| PATCH | `/stocktakes/:id` | 更新备注（仅盘点中） |
| POST | `/stocktakes/:id/counts` | 逐批录入实盘数（可成批，不调库存） |
| POST | `/stocktakes/:id/submit` | 提交核销（必须带 `Idempotency-Key`） |
| POST | `/stocktakes/:id/cancel` | 取消并解冻 |

创建盘点单：

```json
{ "locationId": "uuid", "notes": "9 月末盘点" }
```

服务端把该库位下全部未归档批次按当前账面生成行项目快照。同一库位同时只能有一张盘点中单据（`LOCATION_ALREADY_FROZEN`，409）。

录入实盘数（每批数量为该批次库存单位下的非负十进制，可一次多批）：

```json
{
  "version": 1,
  "items": [
    { "batchId": "uuid", "countedQuantity": "130.5" },
    { "batchId": "uuid", "countedQuantity": "40" }
  ]
}
```

录入只保存实盘数并按 `差异 = 实盘 − 账面快照` 实时算出差异，**不修改库存**。提交核销：

```json
{ "version": 2, "notes": "差异已与负责人确认" }
```

提交在单事务内逐批重算差异：盘盈写 `STOCKTAKE_IN` 流水并入库，盘亏写 `STOCKTAKE_OUT` 流水并出库，账面快照与实盘一致则不产生流水；行项目置为 `RECONCILED`，单据置为 `COMPLETED` 并解冻。存在未录入批次时返回 `STOCKTAKE_INCOMPLETE`（409）。

冻结期间该库位批次的调整、消耗、撤销、移动、归档都会被拒绝：业务入口返回 `LOCATION_FROZEN`（409），数据库触发器（自定义 `SQLSTATE P0999`）是最终防线；其它库位不受影响。核销事务通过仅本事务有效的 `SET LOCAL handcraft.bypassing_freeze='on'` 放行自身写入。

幂等与审计：提交必须携带 `Idempotency-Key`，同一键重复提交只回放首次核销结果，**不会二次调整库存，也不会再写审计**；单据一旦 `COMPLETED` 即终态，不能再录入、再提交或取消。每次创建/冻结、录入、提交、取消、解冻都追加只增审计日志。
